...
public class DivisionDAO {
    ...
    public List<Division> getListOfDivision(String companyName) {
        String SQL = "SELECT divisionname as name, count(divisionname) as numberOfEmployees FROM division d, data_gomelrw dg, pred where dg.id_pred=pred.id and dg.id_division=d.id and pred.namepred=?  group by divisionname order by divisionname";
